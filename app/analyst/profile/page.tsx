...
export default async function ProfilePage() {
  ...
  const history = await prisma.comparison.findMany({ where: { userId }, select: { winner: true } })
    ...
    <ProfileClient
      ...
      stats={{
        ...
        joined: joinedDate?.createdAt
          ? new Date(joinedDate.createdAt).toLocaleDateString('id-ID', { month: 'short', year: 'numeric' })
          : '—',
      }}
    />
  )
